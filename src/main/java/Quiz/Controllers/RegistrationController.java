...
//	       //can also write the photo to local storage
		...
		try {
			PreparedStatement ps = con
					.prepareStatement("INSERT INTO users(username,password,email,mobile,city) values(?,?,?,?,?)");

			// String sql = "INSERT INTO users(username,password,email,mobile,address)
			// values ('"+username+"','"+password+"','"+email+"','"+mobile+"','"+city+"')";
